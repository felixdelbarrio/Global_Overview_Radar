...
cloudrun-config:
	...
	grep -vE '^(GCP_PROJECT|GCP_REGION|BACKEND_SERVICE|FRONTEND_SERVICE|FRONTEND_SA|AUTH_GOOGLE_CLIENT_ID|AUTH_ALLOWED_EMAILS|CORS_ALLOWED_ORIGIN_REGEX)=' "$$ENV_FILE" > "$$TMP_FILE" || true; \
...
cloudrun-env:
	...
	TMP_FILE="$$ENV_FILE.tmp"; \
	awk '$$0 !~ /^(AUTH_ENABLED|AUTH_GOOGLE_CLIENT_ID|AUTH_ALLOWED_EMAILS|GOOGLE_CLOUD_LOGIN_REQUESTED|CORS_ALLOWED_ORIGIN_REGEX)=/' "$$ENV_FILE" > "$$TMP_FILE"; \
	mv "$$TMP_FILE" "$$ENV_FILE"; \
