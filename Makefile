...
cloudrun-config: ensure-cloudrun-env
	...
	grep -vE '^(GCP_PROJECT|GCP_REGION|BACKEND_SERVICE|BACKEND_SA|FRONTEND_SERVICE|FRONTEND_SA|GOOGLE_CLOUD_LOGIN_REQUESTED|AUTH_GOOGLE_CLIENT_ID|AUTH_ALLOWED_EMAILS|[A-Z0-9_]*ORIGIN_REGEX)=' "$$ENV_FILE" > "$$TMP_FILE" || true; \
...
cloudrun-env: ensure-cloudrun-env
	...
	TMP_FILE="$$ENV_FILE.tmp"; \
	awk '$$0 !~ /^(AUTH_GOOGLE_CLIENT_ID|AUTH_ALLOWED_EMAILS|GOOGLE_CLOUD_LOGIN_REQUESTED|[A-Z0-9_]*ORIGIN_REGEX)=/' "$$ENV_FILE" > "$$TMP_FILE"; \
	mv "$$TMP_FILE" "$$ENV_FILE"; \
