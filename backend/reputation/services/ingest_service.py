...
class ReputationIngestService:
    ...
    def _filter_noise_items(
        ...
        if dropped_actor or dropped_actor_text or dropped_guard or dropped_noise:
            total_dropped = dropped_actor + dropped_actor_text + dropped_guard + dropped_noise
            notes.append(
                "filter: dropped %s items (missing_actor=%s, actor_text=%s, guard=%s, noise=%s)"
                % (total_dropped, dropped_actor, dropped_actor_text, dropped_guard, dropped_noise)
            )
        return filtered
