"""Configuracion de logging para bugresolutionradar."""
...
from bugresolutionradar.config import BUG_ENV_PATH, REPO_ROOT, Settings
...
def _resolve_path(file_name: str) -> Path:
    name = file_name.strip() or "bugresolutionradar.log"
    path = Path(name)
    if path.is_absolute():
        return path
...
def _env_mtime() -> Optional[float]:
    try:
        return BUG_ENV_PATH.stat().st_mtime
...
def configure_logging(force: bool = False) -> None:
    """Configura logging leyendo la config actual desde .env."""
...
def get_logger(name: str | None = None) -> logging.LoggerAdapter:
    ...
    configure_logging()
    return _HotReloadingLoggerAdapter(logging.getLogger(name or "bugresolutionradar"), {})
